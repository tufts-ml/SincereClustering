...
def get_parser():
    ...
    parser.add_argument('--eval_funcs', nargs='+',
                        help='Which eval functions to use', default=['v2', 'v2b'])
...
def main(args):
    ...
    train_sampler = DistributedWeightedSampler(
        train_dataset, sample_weights, num_samples=len(train_dataset))
    ...
    unlabelled_train_loader = DataLoader(
        unlabelled_train_examples_test, num_workers=args.num_workers, batch_size=256,
    ...
    for epoch in range(args.epochs):
        ...
        train(model, train_loader, optimizer, fp16_scaler,
              exp_lr_scheduler, cluster_criterion, epoch, args)

        unlabelled_train_sampler.set_epoch(epoch)
        if dist.get_rank() == 0:
            args.logger.info('Testing on unlabelled examples in the training data...')
        all_acc, old_acc, new_acc = test(
            model, unlabelled_train_loader, epoch=epoch, save_name='Train ACC Unlabelled',
            args=args)
