...
class TempCELoss(nn.CrossEntropyLoss):
    def __init__(self, temperature=0.1, **kwargs) -> None:
        super().__init__(**kwargs)
        self.temperature = temperature

    def forward(self, input, target):
        """CrossEntropyLoss with temperature dividing input
...
class TempMeanEntropyLoss(nn.Module):
    def __init__(self, temperature=0.1) -> None:
        super().__init__()
        self.temperature = temperature

    def forward(self, logits: torch.Tensor):
        """Mean prediction entropy loss

        Args:
            logits (torch.Tensor): (B, C) B logits for classification with C classes
